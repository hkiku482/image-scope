...
#[tauri::command]
pub fn write_history(app_handle: tauri::AppHandle, path: PathBuf) -> Result<(), String> {
    ...
    // パスを文字列として保存
    fs::write(&history_path, path.to_string_lossy().as_bytes())
...
#[tauri::command]
pub fn read_history(app_handle: tauri::AppHandle) -> Result<PathBuf, String> {
    let history_path = get_history_path(&app_handle)?;
